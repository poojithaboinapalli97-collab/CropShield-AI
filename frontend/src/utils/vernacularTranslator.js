...
export const getVernacularAdvisory = (langCode, diseaseInfo = {}, customSteps = []) => {
  ...
  if (langCode === 'en') {
    ...
    if (cleanDisease.includes('healthy')) {
      return {
        greeting: 'Hello Farmer. Welcome to CropShield AI Crop Protection Advisory.',
        diagnosisText: `Diagnostic scan detected healthy foliage on your ${crop} with ${confidence}% confidence. Risk Level: Low Risk.`,
        practicalSummary: `Your ${crop} crop is currently in healthy condition. Continue good agricultural practices.`,
        spokenSteps: [
          'No chemical fungicide or bactericide spray required. Continue routine organic field maintenance.',
          'Apply well-decomposed Farm Yard Manure (FYM) or vermicompost @ 2-3 tons/acre for balanced soil nutrition.',
          'Maintain optimal irrigation intervals (drip/furrow) to prevent moisture and waterlogging stress.',
          'Conduct weekly visual field scouting to catch any early foliar spots or sucking pest ingress.',
        ],
        conclusion: 'Regular field scouting ensures high crop yield and early threat detection.',
      };
    }

    if (cleanDisease.includes('bacterial') || cleanDisease.includes('spot')) {
      return {
        greeting: 'Hello Farmer. Welcome to CropShield AI Crop Protection Advisory.',
        diagnosisText: `Diagnostic scan detected Bacterial Leaf Spot on your ${crop} with ${confidence}% model confidence. Risk Level: ${risk}.`,
        practicalSummary: 'Bacterial foliar infection with water-soaked lesions observed across crop leaves.',
        spokenSteps: [
          'Spray Copper Oxychloride 50% WP @ 2.5 g/L + Streptocycline 100 ppm (1g in 10L water).',
          'Stop overhead sprinkler watering immediately to prevent bacterial splash between plant rows.',
          'Sanitize pruning secateurs and equipment with 1% sodium hypochlorite solution.',
          'Apply bio-fungicide Bacillus subtilis or Trichoderma viride @ 5 g/L around root zone.',
        ],
        conclusion: 'For complex cases, escalate directly to accredited KVK Pathology laboratories.',
      };
    }

    if (cleanDisease.includes('curl') || cleanDisease.includes('virus')) {
      return {
        greeting: 'Hello Farmer. Welcome to CropShield AI Crop Protection Advisory.',
        diagnosisText: `Diagnostic scan detected Leaf Curl Virus on your ${crop} with ${confidence}% model confidence. Risk Level: ${risk}.`,
        practicalSummary: 'Viral leaf curling and vector pressure identified across young vegetative flush.',
        spokenSteps: [
          'Install 15 yellow and 15 blue sticky traps per acre to capture thrips and whitefly vectors.',
          'Spray Diafenthiuron 50% WP @ 1.2 g/L or Fipronil 5% SC @ 2.0 ml/L against thrips and yellow mite vectors.',
          'Rogue out severely stunted, upward-cupped plants to stop field-wide virus transmission.',
          'Spray Neem oil 10,000 ppm @ 2.0 ml/L as an organic repellent on fresh terminal flushes.',
        ],
        conclusion: 'Vector suppression in early crop stages protects entire seasonal yield.',
      };
    }

    if (cleanDisease.includes('late blight')) {
      return {
        greeting: 'Hello Farmer. Welcome to CropShield AI Crop Protection Advisory.',
        diagnosisText: `Diagnostic scan detected Late Blight on your ${crop} with ${confidence}% model confidence. Risk Level: High Risk.`,
        practicalSummary: 'Aggressive Phytophthora blight lesions identified on foliage. Immediate intervention required.',
        spokenSteps: [
          'Emergency Action: Spray Cymoxanil 8% + Mancozeb 64% WP @ 2.0 g/L or Dimethomorph 50% WP @ 1.0 g/L immediately.',
          'In overcast/wet weather, rotate after 7 days with Metalaxyl-M 4% + Mancozeb 64% WP (Ridomil Gold) @ 2.5 g/L.',
          'Stop overhead sprinkler irrigation; ensure soil drains freely to prevent water-soaked foliar decay.',
          'Apply bio-control agent Trichoderma harzianum @ 5 g/L around root zone after chemical interval.',
        ],
        conclusion: 'Late blight spreads rapidly in humid weather; spray within 24 hours of symptom onset.',
      };
    }
    ...
    if (cleanDisease.includes('rust')) {
      return {
        greeting: 'Hello Farmer. Welcome to CropShield AI Crop Protection Advisory.',
        diagnosisText: `Diagnostic scan detected Foliar Rust on your ${crop} with ${confidence}% model confidence. Risk Level: ${risk}.`,
        practicalSummary: 'Fungal rust pustules rupturing leaf epidermis identified across canopy.',
        spokenSteps: [
    ...
    return {
      greeting: 'Hello Farmer. Welcome to CropShield AI Crop Protection Advisory.',
      diagnosisText: `Diagnostic scan detected ${disease} on your ${crop} with ${confidence}% model confidence. Risk Level: ${risk}.`,
      practicalSummary: `Targeted crop protection actions recommended for ${crop}.`,
